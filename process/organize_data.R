...
data.bsl = read.table(DATA_FILE.bsl, header=TRUE, sep=',') %>% mutate(Subject = nspnID, cohort = study_primary) %>% dplyr::select(Subject, 
                                                                                               age_iua1, 
                                                                                               cohort, 
                                                                                               medical_cond, 
                                                                                               medical_specify,
                                                                                               ...
                                                                                               age_scan1, 
                                                                                               sex) 
data.fu = read.table(DATA_FILE.fu, header=TRUE, sep='\t') %>% mutate(Subject = SubjectID) %>% dplyr::select(Subject,
                                                                                                    age_scan2, 
                                                                                                    centre_scan2)


# fix missing data
data.bsl[data.bsl$Subject == 21741, ]$age_scan1 = data.bsl[data.bsl$Subject == 21741, ]$age_iua1
  
  
data.decAc = read.table(DATA_FILE.decAc, header=TRUE, sep=',') %>% mutate(Subject = nspnID) %>% 
  rename(decAc.bsl = decAc4b.bsl, decAc.fu = decAc4b.fu2) %>% dplyr::select(Subject, decAc.bsl, decAc.fu) 

# retrieve IQ values
data.IQ = read.table(DATA_FILE.IQ, header=TRUE, sep=',') %>% mutate(Subject = subjectID)
data.IQ$IQcomp = as.vector(0.5*(scale(data.IQ$vocabulary_raw_score) + scale(data.IQ$matrix_raw_score))) 

IQ.bsl = subset(data.IQ, measurement == "iua_baseline_arm_1") [c("Subject", "IQcomp")]
IQ.fu = subset(data.IQ, measurement == "iua_1st_follow_up_arm_1") [c("Subject", "IQcomp")]
IQ = merge(IQ.bsl, IQ.fu, by = "Subject", all = T, suffixes = c(".bsl", ".fu"))

data.pars.NSPN = merge(data.fu, data.bsl, by = "Subject", all = T)
data.pars.NSPN = merge(data.pars.NSPN, data.decAc, by = "Subject")
data.pars.NSPN = merge(data.pars.NSPN, IQ, by = "Subject")
...
# take only 2k cohort
data.pars.2k = subset(data.pars.NSPN, cohort == '2K_Cohort')
PARAMS_DATA_FILE_NSPN = '~/Data/gng_modelling/NSPN/params_NSPN_2k.csv'
write.table(data.pars.2k, PARAMS_DATA_FILE_NSPN, sep = ';')

# non-depressed
data.pars.nondep = data.pars.2k[-grep("epression", data.pars.2k$medical_specify), ]
PARAMS_DATA_FILE_NSPN = '~/Data/gng_modelling/NSPN/params_NSPN_nondep.csv'
...
have_scan = !is.na(data.pars.nondep$age_scan1) & !is.na(data.pars.nondep$age_scan2)
...
data.pars.nondep.split[have_scan, c("decAc.bsl", "IQcomp.bsl", "age_scan1")][index, ] = NA 
data.pars.nondep.split[have_scan, c("decAc.fu", "IQcomp.fu", "age_scan2")][!index, ] = NA 

data.pars.nondep.long = rbind(
  data.pars.nondep %>% filter (!is.na(decAc.bsl)) %>% dplyr::select(Subject, centre_scan1, age_scan1, cohort, medical_cond, medical_specify, sex, sex.num,
                              decAc.bsl, IQcomp.bsl, CBU.1, UCL.1) %>% rename(centre_scan = centre_scan1, age_scan = age_scan1, decAc = decAc.bsl, 
                                                                              IQcomp = IQcomp.bsl, CBU = CBU.1, UCL = UCL.1) %>% mutate(group = "bsl"),
  data.pars.nondep %>% filter (!is.na(decAc.fu)) %>% dplyr::select(Subject, centre_scan2, age_scan2, cohort, medical_cond, medical_specify, sex, sex.num, 
                              decAc.fu, IQcomp.fu, CBU.2, UCL.2) %>% rename(centre_scan = centre_scan2, age_scan = age_scan2, decAc = decAc.fu, 
                                                                             IQcomp = IQcomp.fu, CBU = CBU.2, UCL = UCL.2) %>% mutate(group = "fu")
)
...
sum(complete.cases(mydata[c('Subject','age_scan1','sex','decAc.bsl', 'IQcomp.bsl')]))

# ica data
#FILE_SUBJECTS = file.path(DIR_ICA_ME, 'subjects.txt')
#subjects = as.numeric(read.table(FILE_SUBJECTS))
#mm = mydata[complete.cases(mydata[c('Subject','age_scan1','sex','decAc.bsl')]), ]

# w followup
sum(complete.cases(mydata[c('Subject','age_scan1','age_scan2','sex','decAc.bsl', 'decAc.fu')]))
sum(complete.cases(mydata[c('Subject','age_scan1','age_scan2','sex','decAc.bsl', 'decAc.fu', 'IQcomp.bsl', 'IQcomp.fu')]))
...
write.table(t(c(unlist(read.table('~/Data/NSPN/ICA_ME/nspn_ME/ica200.gica/subjects.txt')),
unlist(read.table('~/Data/NSPN/ICA_ME/nspn_ME_fu/ica200.bsl.gica/subjects.txt')))),
